...
from path_datasets_and_weights import path_chest_imagenome, path_mimic_cxr, path_mimic_cxr_jpg, path_full_dataset
...
def check_coordinate(coordinate: int, dim: int) -> int:
    ...
    """
    coordinate = int(coordinate/224 * dim)
    return coordinate
...
def coordinates_faulty(height, width, x1, y1, x2, y2) -> bool:
    ...
    return area_of_bbox_is_zero or smaller_than_zero 
...
def get_report(subject_id, data):
    for i in range(len(data['train'])):
        if data['train'][i]['id'] == subject_id:
            report = data['train'][i]['report']
            return report
    return -1
...
def get_reference_report(subject_id: str, study_id: str, missing_reports: list[str]):
    ...
    if not os.path.exists(path_to_report):
        print(f"Report not found: {path_to_report}")
...
def get_rows(dataset: str, path_csv_file: str, image_ids_to_avoid: set) -> list[list]:
    ...
    with open('/Users/claire/Desktop/Stuff-/codes/dissertation/data/mimic_cxr/annotation.json', 'r') as f:
    ...
    with open(path_csv_file) as csv_file:
        ...
        for row in tqdm(csv_reader, total=total_num_rows):
            ...
            if image_id in IMAGE_IDS_TO_IGNORE or image_id in image_ids_to_avoid:
                print(f"Image {image_id} is in IMAGE_IDS_TO_IGNORE or image_ids_to_avoid, so it is ignored.")
            ...
            if dataset in ["valid", "test"]:
                ...
                reference_report = get_report(image_id, report_data)
            ...
            if not os.path.exists(chest_imagenome_scene_graph_file_path):
                print(chest_imagenome_scene_graph_file_path + 'not exist')
                continue
